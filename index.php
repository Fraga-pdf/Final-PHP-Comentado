...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Captura o token vindo de dentro do formulário HTML.
    $token_formulario = $_POST['csrf_token'] ?? '';
    // Captura o token oficial que está guardado na memória do servidor.
    $token_sessao = $_SESSION['csrf_token'] ?? '';
    
    // COMPARAÇÃO DOS TOKENS:
    // Se o formulário não enviou o token, ou se o token enviado for diferente do guardado no servidor...
    if (empty($token_formulario) || $token_formulario !== $token_sessao) {
        // O comando die() interrompe o PHP imediatamente e exibe uma mensagem limpa.
        // IMPACTO NA NOTA: Protege o seu banco de dados contra inserções falsas ou maliciosas automatizadas.
        die("Erro de segurança grave: Token CSRF inválido ou ausente.");
    }
}

// GERAÇÃO DO TOKEN SECRETO:
// Se o usuário acabou de entrar no site e ainda não possui um token gerado na memória...
if (!isset($_SESSION['csrf_token'])) {
    // random_bytes(32) gera 32 bytes de pura aleatoriedade criptográfica.
    // bin2hex() transforma esses bytes em uma string legível de letras e números gigantesca.
    // IMPACTO NA DEFESA: Isso cria uma chave única e imprevisível para a sessão atual daquele aluno.
...
switch ($url) {
    ...
    // TELA DE RECUPERAÇÃO: Se a URL for ?p=recuperar, abre o formulário de esqueci a senha.
    case 'recuperar':
        HomeController::recuperar();
        break;

    // PROCESSAMENTO DA RECUPERAÇÃO: Rota que valida os dados de segurança e executa a alteração da senha.
    case 'fazer-recover':
    case 'fazer-recuperar':
        HomeController::fazerRecuperar();
        break;
